...
def generateTransitionMatrix(samples):
    ...
    pairCountMap = pairSamples.countByValue()#返回的是一个新的key,value对，其中key为samples平铺以后的键值对，value是对应的个数  如：[((858,50),1),((50,593),1)]
    pairTotalCount = 0
    transitionCountMatrix = defaultdict(dict)#相当于Java里面getOrDefault,没有值的都是空dict
    itemCountMap = defaultdict(int)#创建字典，没有值得都是0
    for key, cnt in pairCountMap.items():
        key1, key2 = key
        transitionCountMatrix[key1][key2] = cnt#这就是双map,第一层map记录键值对的数量，第二层map记录键值对的转移概率
        itemCountMap[key1] += cnt#存的是itemid 和出度对
        pairTotalCount += cnt
    ...
    for key1, transitionMap in transitionCountMatrix.items():
        for key2, cnt in transitionMap.items():
            transitionMatrix[key1][key2] = transitionCountMatrix[key1][key2] / itemCountMap[key1]#cij/出度(i)[(850,(50,0.1))]
    for itemid, cnt in itemCountMap.items():
        itemDistribution[itemid] = cnt / pairTotalCount#当前id出度除以所有出度，就是当前id分布[(850,0.1),(),()]
